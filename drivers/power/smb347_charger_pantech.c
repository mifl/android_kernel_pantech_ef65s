...
#if defined(CONFIG_PANTECH_PMIC_CHARGE_LIMIT_WITH_LCD) || defined(CONFIG_PANTECH_PMIC_INPUT_CURRENT_LIMIT_WITH_LCD)
...
#define NORMAL_USBIN 0x16 //USBIN 1.8A
#define NORMAL_FAST  0xB9 // FAST 2A, term 50mA
#define LCD_ON_LIMIT_USBIN 0x14 //limit USBIN 1200mA
...
#if defined(CONFIG_PANTECH_PMIC_INPUT_CURRENT_LIMIT_WITH_LCD)
void set_input_current_limit_lcd_on(int mode)
{
	if (smb_data_ext.t_sensor_mode == false)
	{
		if (smb_data.lcd_state.state) {//LCD on
			smb349_write_reg(0x01, LCD_ON_LIMIT_USBIN); //1200mA
		} else {//LCD off
			//reset state
			smb349_write_reg(0x30, 0x81); //charging disable
			smb349_write_reg(0x31, 0x02); //usb mode

			switch(mode){ // all 1.8A
...
#if defined(CONFIG_PANTECH_PMIC_INPUT_CURRENT_LIMIT_WITH_LCD)
		if (!get_is_offline_charging_mode()) {
			set_input_current_limit_lcd_on(BATT_THERM_COOL);
		} else {
			smb349_write_reg(0x01, WARM_COOL_USBIN);
		}
#else
...
#if defined(CONFIG_PANTECH_PMIC_INPUT_CURRENT_LIMIT_WITH_LCD)
		if (!get_is_offline_charging_mode()) {
			set_input_current_limit_lcd_on(BATT_THERM_NORMAL);
		} else {
			smb349_write_reg(0x01, NORMAL_USBIN);
		}
#else
...
#if defined(CONFIG_PANTECH_PMIC_INPUT_CURRENT_LIMIT_WITH_LCD)
		if (!get_is_offline_charging_mode()) {
			set_input_current_limit_lcd_on(BATT_THERM_WARM);
		} else {
			smb349_write_reg(0x01, WARM_COOL_USBIN);
		}
#else
...
#endif /* CONFIG_PANTECH_PMIC_INPUT_CURRENT_LIMIT_WITH_LCD */
